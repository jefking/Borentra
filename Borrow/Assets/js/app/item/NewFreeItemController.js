(function ($) {
  var form = $("#quick-new-free-item-form");
  
  ResetForm = function () {

    form.find("#item-description").val("");
    form.find("#item-share-privacy").val("0");
    form.find("#item-trade-privacy").val("0");
    form.find("#item-rent-privacy").val("0");
    form.find("#item-free-privacy").val("1");
  ...
  $("body a.btn-dialog-new-free-item").on("click", function (e) {
    e.preventDefault();
    GlobalNewFreeItemCommand();
  });

})(jQuery);

// This is used as a way for key bindings to invoke the dialog
var GlobalNewFreeItemCommand = function () {
  var form = $("#quick-new-free-item-form");

  //ResetForm();

  $("#dialogNewFreeItem").modal({ backdrop: 'static' }).css({
    "width": 700,
