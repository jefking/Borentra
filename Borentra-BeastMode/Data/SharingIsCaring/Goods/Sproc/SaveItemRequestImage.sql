CREATE PROCEDURE [Goods].[SaveItemRequestImage]
...
BEGIN
	...
	BEGIN
		...
		MERGE [Goods].[ItemRequestImage] AS Images
			USING (
			...
			ON [Images].[Identifier] = [NewData].[Identifier]
				AND [Images].[ItemRequestIdentifier] = [NewData].[ItemRequestIdentifier]
			WHEN MATCHED
			THEN UPDATE
			...
			EXECUTE [Goods].[SearchItemRequestImage]
